 ...
 * Tiebreakers (in order): conference record → H2H (within tied group) → point differential → points for.
...
export function calcSeeds(teams, scores) {
  ...
  Object.values(byConf).forEach(names => {
    ...
    Object.keys(byW).sort((a, b) => Number(b) - Number(a)).forEach(w => {
      ...
      // H2H record within the tied group only
      const h2hW = {};
      group.forEach(n => { h2hW[n] = 0; });
      (scores || []).forEach(g => {
        if (!g.s1 || !g.s2) return;
        if (!group.includes(g.t1) || !group.includes(g.t2)) return;
        const s1 = parseInt(g.s1, 10), s2 = parseInt(g.s2, 10);
        if (s1 > s2) h2hW[g.t1] = (h2hW[g.t1] || 0) + 1;
        else if (s2 > s1) h2hW[g.t2] = (h2hW[g.t2] || 0) + 1;
      });

      group.sort((a, b) => {
        const hd = (h2hW[b] || 0) - (h2hW[a] || 0);
        if (hd !== 0) return hd;
